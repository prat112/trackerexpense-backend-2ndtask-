...
window.addEventListener("DOMContentLoaded",async()=>{
    try{
     ...
     const response=await axios.get(`http://54.210.69.239:3100/expense?page=${page}&size=${size}`,{ headers:{"Authorization":token}})

     for(var i=0;i<response.data.expenseData.length;i++)
         showExp(response.data.expenseData[i]);
         showpagination(response.data);

    const history=await axios.get("http://54.210.69.239:3100/user/dhistory",{ headers:{"Authorization":token}})
...
{   try{
    ...
    let myobj={
      amount,description,category
        ...
        const response=await axios.post("http://54.210.69.239:3100/expense/add-expense",myobj,{headers:{"Authorization":token}})
...
async function showExp(myobj)
{
    ...
    deletebtn.addEventListener('click',async function(){
        try{
            const token=localStorage.getItem('token');
            const dId=myobj.id;
            await axios.delete(`http://54.210.69.239:3100/expense/delete-expense/${dId}`,{headers:{"Authorization":token}})
...
async function paymentfunc(e){
    const token=localStorage.getItem('token');
    const response=await axios.get(`http://54.210.69.239/purchase/premiummembership`,{headers:{"Authorization":token}});
    var options={
        "key":response.data.key_id,
        "order_id":response.data.order.id,
        "handler":async function(response){
           const res= await axios.post(`http://54.210.69.239:3100/purchase/updatetrstatus`,{
    ...
    rzp1.on('payment.failed',async function(response){
            await axios.post(`http://54.210.69.239:3100/purchase/updatetrstatus`,{
            order_id:options.order_id,
            payment_id:response.razorpay_payment_id},
            {headers:{"Authorization":token}})
...
async function showleaderBoard(){
    if(clicked){
        clicked=false;
        const token=localStorage.getItem('token');
        const lbArray=await axios.get(`http://54.210.69.239:3100/premium/showleaderboard`,{headers:{"Authorization":token}})
...
async function download(){
    try{
        ...
        const token=localStorage.getItem('token');
        const response=await axios.get('http://54.210.69.239:3100/user/download', { headers: {"Authorization" : token} });
